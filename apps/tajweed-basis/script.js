(function () {
  ...
  function renderGallery(filter){
    ...
    for(var i=0;i<RULES.length;i++){
      ...
      html += '<article class="rule-card'+(isLearned(r.n)?' learned':'')+(isDifficult(r.n)?' difficult':'')+'" data-index="'+i+'" style="--cat-color:'+catColor(r.category)+';animation:cardStagger .4s cubic-bezier(.22,1,.36,1) '+(.03*count)+'s both">'+
        '<span class="rule-num">Regel '+r.n+'</span><span class="rule-cat" style="background:'+catColor(r.category)+';color:#fff">'+escapeHtml(r.category)+'</span><p class="rule-title">'+escapeHtml(r.title)+'</p><p class="rule-keyword">'+escapeHtml(r.keyword)+'</p><p class="rule-arabic" dir="rtl" lang="ar">'+escapeHtml(r.ar)+'</p></article>';
      count++;
    }
  ...
  function renderFlashcard(){
    ...
    fcNumber.textContent='Regel '+r.n; fcCategory.textContent=r.category; fcCategory.style.background=catColor(r.category); fcCategory.style.color='#fff'; fcTitle.textContent=r.title; fcArabic.textContent=r.ar;
  ...
  function openModal(r){
    modalNumber.textContent='Regel '+r.n; modalCategory.textContent=r.category; modalCategory.style.color=catColor(r.category); modalTitle.textContent=r.title; modalArabic.textContent=r.ar;
